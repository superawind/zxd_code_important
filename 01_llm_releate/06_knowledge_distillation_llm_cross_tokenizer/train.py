...
class ULDLoss(nn.Module):
    ...
    # 函数作用，找到 student、teacher 模型的词表中，共有的词，并且返回共有词 对应的 token id  的映射 
    # vocab_mapping -> {'teacher_token_id': 'studnet_token_id'}
    ...
    # 该函数的作用是将 每个组 的 概率进行乘积， 主要为了对齐 student 和 teacher 的 seq_len 不同问题
    ...
    # 作用是，解决 vocab_size 大小不同问题，解决方式： 分别对相同的 token 和 不同的 token 进行计算，相同的token 类似（同系列模型计算kl散度），不同的token 则只填充到相同的长度后，计算整体分布差异
