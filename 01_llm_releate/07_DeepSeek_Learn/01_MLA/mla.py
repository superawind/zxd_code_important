...
class MLA(nn.Module):
    def __init__(self,
        ...
        # hidden_states 经过这个权重，同时获得 Ct_kv、Kt_R
    ...
    def forward(self, x, start_pos: int, mask=None):
        ...
        if self.mode == 'naive':
            # 传统的 mha，不过此时的 位置编码采用额外的一些维度实现，q, k矩阵都多了一部分q_ke, k_pe 
            ...
            # mla , k_pe 之前为了计算旋转位置编码，进行了 unsqueeze，此处还原回去
            k_pe = k_pe.squeeze(2)
            ...
            # 计算前 q_nope shape:[bs, seq_len, n_heads, qk_nope_head_dim]  wkv_b[:, :self.qk_nope_head_dim].shape : [n_heads, qk_nope_head_dim, kv_lora_rank]
            # 第一个元素表示参与计算前后形状的变化，计算后的q_nope 等价于 经过 Wq_u * Wk_u(T) 的吸收矩阵，这样 c==kv 就相当于是 传统的k
        ...
        # 注意：mla 此处应该是 wkv_b 和 wo 吸收了，但是此处仍然是分开计算的。
        x = self.wo(x)  
