...
class Config():
    def __init__(self,
                llm_model_path = '/opt/users/models/Qwen2.5-0.5B-Instruct',
...
class MTPModule(nn.Module):
    # 输入是 上一阶段的 hidden_states 和 前一个 token 的cat，所以维度为 2 * hidden_state
    # 自定义一个简化版本的 mtp ，不使用 deepseek 文章中的 transformer block 
...
class MTP(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, **kwargs):
        ...
        for head_index in range(0, self.config.predict_tokens_num-1):  # 当前 token 原生会预测下一个，因此需要减1，
    ...
    def generate(self,input_ids,max_length, **kwargs):
        ...
        with torch.no_grad():
            ...
            while seq.size(1) < max_length:
                ...
                # 将新序列输入main模型(验证模型)进行验证，保留符合条件的token，类似推测解码，mtp 模块产生的就是小模型的结果，main模块用来验证
                _, all_logits = self.forward_main(all_tokens)
                
                # 取出需要验证的token对应的logits [batch_size, spec_tokens_len, vocab_size]
                ...
                # 拼接各个token的生成概率
                accept_probs = torch.cat(accept_probs, dim=-1) # [bs, spec_token_num]
                
                # 保留概率大于阈值的token，接受这部分token，否则舍弃（舍弃某个token时，后面的token都要舍弃）
                # 接受token的掩码
...
def train(config: Config, model, dataloader, optimizer, writer, device, epochs, print_step, save_step, save_path):
    steps = 0
    model.train()
    for epoch in range(epochs):
        for step, batch in enumerate(dataloader):
            ...
            for index in range(0, config.predict_tokens_num-1):
                ...
                # 实际上感觉这一点应该直接从 input 改，mtp 的 labels 为原本 labels[:, index+2:, :] 对应 input 是原本 input[:, index+1:-1, :]，其中index是mtp 模块的索引
                mtp_head_output = mtp_head_output[:, :-(1+index+1)]
                mtp_head_output = mtp_head_output.reshape(-1, model.main_modle.config.vocab_size)
...
class MyDataCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
            features 是一个大小为 batch 的列表，每一个元素都是 getitem 中取出来的一个元素
        """
...
if __name__ == '__main__':
    """
    给 qwen2.5-0.5B-Instruct 模型增加 MTP 结构
    """
    # 日志记录
    ...
    dataloader = DataLoader(dataset=dataset, batch_size=8, shuffle=True, num_workers=8, collate_fn=MyDataCollator(AutoTokenizer))
    
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    save_path = '/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/deepseek_mtp'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    train(config, model, dataloader, optimizer, writer, device='cuda', epochs=10, print_step=10, save_step=500, save_path=save_path)