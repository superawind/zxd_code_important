...
if __name__ == '__main__':   
    config = Config()
    model = LLM(config)
    print(f'模型参数量包含：：：{sum((p.numel() for p in model.parameters() if p.requires_grad))}')
    
    data_collator = DefaultDataCollator()
    tokenizer = AutoTokenizer.from_pretrained('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/train_llm/tokenizer', use_fast=True)
    args = TrainingArguments(output_dir='/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/llm_test',
                             num_train_epochs=10,
                             do_train=True,
    ...
    trainer.save_model('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/llm_test/model')
    trainer.save_state()
