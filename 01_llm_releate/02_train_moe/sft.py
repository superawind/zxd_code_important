...
if __name__ == '__main__':
    ...
    model = AutoModelForCausalLM.from_pretrained('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/moe')
    print(f'模型参数量为：：{sum((p.numel() for p in model.parameters() if p.requires_grad))}')
    
    data_collator = DefaultDataCollator()
    tokenizer = AutoTokenizer.from_pretrained('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/moe', use_fast=True)
