...
def repeat_kv(hidden_states, n_rep):
    batch, nseq, num_key_value_heads, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states = hidden_states[:, :, :, None, :].expand(batch, nseq, num_key_value_heads, n_rep, head_dim)
    return hidden_states.reshape(batch, nseq, num_key_value_heads * n_rep, head_dim)
...
def load_balancing_loss_func(
    ...
    # [layers * bs * seq_len, top_k]
    _, selected_experts = torch.topk(routing_weights, top_k, dim=-1)
    # [layers * bs * seq_len, top_k, num_experts]
...
class Config(PretrainedConfig):
    model_type = 'moe_model'  # 此处定义的model_type 就是最后保存模型的 config.json 中的 model_type 字段
...
# 自定义的结构，类名 LLM ,就是最后保存在 config.json 中的  architecture
class LLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.output = nn.Linear(self.config.hidden_size, self.vocab_size, bias=False)
...
if __name__ == '__main__':
    ...
    tokenizer = AutoTokenizer.from_pretrained('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/train_moe/tokenizer', use_fast=True)
    args = TrainingArguments(output_dir='/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/moe_pretrain',
                             num_train_epochs=10,
                             do_train=True,
    ...
    dataset = LLMDataset('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/data/pretrain.jsonl', tokenizer=tokenizer, max_seq_len=512)
    trainer = Trainer(model=model, args=args, train_dataset=dataset, tokenizer=tokenizer, data_collator=data_collator)
    # 如果是初次训练resume_from_checkpoint为false，接着checkpoint继续训练，为True
    trainer.train(resume_from_checkpoint=True)
    trainer.save_model('/mnt/code/zhaoxudong03/RL/verl_base_zxd/01_llm_releate/save_models/pretrain')
    trainer.save_state()
